...
class Auth
{
    ...
    public function check($name, $uid, $relation = 'or')
    {
        ...
        foreach ($authList as $val) {
            if (in_array(strtolower($val), array_map('strtolower', $name)))
                $list[] = $val;
        }
    ...
    public function getGroups($uid)
    {
        ...
        if (isset($groups[$uid]))
            return $groups[$uid];
        $field        = 'a.uid,a.group_id,g.id,g.title,g.status,g.rules';
        $condition    = [
            'a.group_id',
            'g.id'
        ];
        $user_groups  = (new Db())
            ...
            ->join($this->_config['auth_group'], 'g', $condition, 'inner')
            ->where('a.uid', $uid)
            ->field($field)
            ->select();
    ...
    public function getAuthList($uid)
    {
        ...
        // 这里需要修改数据存储方式
        $authListCache = json_decode(Cache::get('auth_list_' . $uid), true);
        if (isset($authListCache)) {
            return $authListCache;
        }
        //读取用户所属用户组
        ...
        $rules = (new Db())->table($this->_config['auth_rule'])->where($map)->select();
        //循环规则，判断结果。
        $authList = [];
        foreach ($rules as $r) {
        ...
        if ($this->_config['auth_type'] == 2) {
            //session结果
            $authListCache = json_encode($authList);
            Cache::set('auth_list_' . $uid, $authListCache); // 这里需要设置缓存时间
//            $_SESSION['_AUTH_LIST_' . $uid] = $authList;
    ...
    protected function getUserInfo($uid)
    {
        ...
        if (!isset($userinfo[$uid])) {
            $userinfo[$uid] = (new Db())->table($this->_config['auth_user'])->where('id', $uid)->find();
        }
        return $userinfo[$uid];
    }
}